...
export const BaggageDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const updateBaggageRecord = useCallback(async (pir: string, updates: Partial<BaggageRecord>) => {
        ...
        setWtReports(currentReports => {
            ...
            const isConfirmed = updates.IsConfirmedByPassenger === true || updates.IsConfirmedByPassenger === 'TRUE';
            const hasAi = !!updates.AiFeatures;
            ...
            return currentReports.map(report => {
                if (report.pir.toUpperCase() === searchPir) {
                    return {
                        ...
                        lastUpdate: new Date(now),
                        isConfirmed: updates.IsConfirmedByPassenger !== undefined ? isConfirmed : report.isConfirmed,
                        hasAiFeatures: updates.AiFeatures !== undefined ? hasAi : report.hasAiFeatures
                    };
                }
                return report;
            });
    ...
    const addBaggageRecord = useCallback((record: BaggageRecord) => {
        ...
        const newReport: BaggageReport = {
            ...
            lastUpdate: new Date(),
            isConfirmed: formattedRecord.IsConfirmedByPassenger === true || formattedRecord.IsConfirmedByPassenger === 'TRUE',
            hasAiFeatures: !!formattedRecord.AiFeatures
        };
