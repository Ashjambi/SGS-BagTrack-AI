...
import { SparklesIcon, CheckCircleIcon, BellIcon } from './common/icons';
...
const ReportCard: React.FC<{ report: BaggageReport; onViewDetails: (report: BaggageReport) => void; }> = ({ report, onViewDetails }) => (
    <Card className="flex flex-col relative group overflow-hidden border-transparent hover:border-cyan-400/50 transition-all duration-300">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
            ...
            {report.isConfirmed && (
                <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-brand-green/10 text-brand-green text-[9px] font-black border border-brand-green/30 uppercase tracking-tighter">
                    <CheckCircleIcon className="w-3 h-3" /> تم التوثيق
                </span>
            )}
            {report.hasAiFeatures && (
                <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-cyan-500/10 text-cyan-400 text-[9px] font-black border border-cyan-400/30 uppercase tracking-tighter">
                    <SparklesIcon className="w-3 h-3" /> بصمة AI
                </span>
            )}
        </div>
...
const StaffView: React.FC = () => {
    ...
    const handleCloseDetail = useCallback(() => setIsDetailModalOpen(false), []);
    const handleCloseCreate = useCallback(() => setIsCreateModalOpen(false), []);
    const handleCloseFound = useCallback(() => setIsFoundBaggageModalOpen(false), []);
    ...
    const handleFullSync = async () => {
        ...
        try {
            ...
            const formatted = globalRecords.map(r => ({
                ...
                isConfirmed: r.IsConfirmedByPassenger === true || r.IsConfirmedByPassenger === 'TRUE',
                hasAiFeatures: !!r.AiFeatures
            }));
    ...
    // ميزة الاستجابة الاستباقية: فرز الحقائب التي قاربت على انتهاء مهلة الـ 24 ساعة (خطر شكوى)
    const riskReports = useMemo(() => {
        const now = new Date().getTime();
        return reportsData.filter(r => {
            if (r.status === 'Resolved' || r.status === 'Delivered') return false;
            const elapsed = (now - r.lastUpdate.getTime()) / (1000 * 3600);
            return elapsed >= 18; // 18 ساعة أو أكثر تعد "خطر مرتفع"
        });
    }, [reportsData]);
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8">
            ...
            {/* انعكاس ميزة الاستجابة الاستباقية في الواجهة */}
            {riskReports.length > 0 && (
                <div className="bg-yellow-500/10 border-2 border-yellow-500/30 p-4 rounded-xl flex items-center justify-between animate-in slide-in-from-top-4 duration-500">
                    <div className="flex items-center gap-4">
                        <div className="p-2 bg-yellow-500 rounded-lg text-brand-gray-dark shadow-[0_0_15px_rgba(234,179,8,0.4)]">
                            <BellIcon className="w-6 h-6" />
                        </div>
                        <div>
                            <h4 className="text-yellow-500 font-black text-sm uppercase">مركز الإنذار المبكر (Proactive Hub)</h4>
                            <p className="text-xs text-gray-300 font-bold">يوجد {riskReports.length} بلاغاً تجاوزت 18 ساعة. يرجى التدخل الفوري لتجنب تصعيد الشكاوى.</p>
                        </div>
                    </div>
                    <button 
                        onClick={() => {
                        ...
                        className="px-4 py-2 bg-yellow-500 text-brand-gray-dark font-black text-[10px] rounded-lg hover:bg-yellow-400 transition-all shadow-lg active:scale-95"
                    >
                        تصفية المخاطر العالية
                    </button>
                </div>
            )}
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="bg-brand-gray-dark p-1 rounded-lg flex border border-cyan-400/30">
                    <button 
                        onClick={() => setDataSource(DATA_SOURCE_MODE.EXCEL)} 
                        className={`px-4 py-2 rounded-md text-xs font-bold transition-all ${dataSource === DATA_SOURCE_MODE.EXCEL ? 'bg-brand-gray text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                    >
                        قاعدة بيانات النظام
                    </button>
                    <button 
                        onClick={() => setDataSource(DATA_SOURCE_MODE.WORLDTRACER)} 
                        className={`px-4 py-2 rounded-md text-xs font-bold transition-all ${dataSource === DATA_SOURCE_MODE.WORLDTRACER ? 'bg-brand-gray text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                    >
                        قاعدة بيانات worldtracer
                    </button>
                </div>
            </div>
