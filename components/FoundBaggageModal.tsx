...
const FoundBaggageModal: React.FC<FoundBaggageModalProps> = ({ isOpen, onClose, onRecordCreate }) => {
    ...
    const handleManualAnalyze = async () => {
        ...
        try {
            ...
            if (analysis.name && analysis.name !== 'null' && analysis.name !== 'Unknown') {
                setPassengerName(analysis.name);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!currentLocation || (!photo1.file && !photo2.file)) {
            alert("يرجى تحديد الموقع والتقاط صورة واحدة على الأقل.");
            return;
        }

        setIsLoading(true);
        try {
            const photoUrl1 = photo1.file ? await base64FromFile(photo1.file) : '';
            ...
            const newRecord: BaggageRecord = {
                ...
                BaggagePhotoUrl: photoUrl1 || photoUrl2, // استخدم الصورة المتاحة كصورة أساسية
                BaggagePhotoUrl_2: photoUrl1 && photoUrl2 ? photoUrl2 : '',
            ...
            <div className="max-w-2xl mx-auto">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className={labelStyle}>الوجه الأول</label>
                        ...
                        <div className="space-y-1">
                            <label className={labelStyle}>الوجه الثاني (اختياري)</label>
                    ...
                    {(photo1.preview || photo2.preview) && (
                        <div className="space-y-2">
                            <button type="button" onClick={handleManualAnalyze} disabled={isAnalyzing} className={`w-full py-2 border rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${isAnalyzing ? 'bg-brand-green/10 text-brand-green border-brand-green/20' : 'bg-brand-green/20 text-brand-green border-brand-green/30 hover:bg-brand-green/30'}`}>
                                {isAnalyzing ? "جاري تحليل زوايا الرؤية..." : <><SparklesIcon className="w-4 h-4" /> تحليل المواصفات آلياً (AI Analysis)</>}
                            </button>
                    ...
                    <div className="p-4 bg-brand-gray-dark/50 border border-brand-gray-light/50 rounded-xl space-y-4">
                        ...
                        <div><label className={labelStyle}>العلامات المميزة</label><textarea value={aiFeatures.distinctiveMarks} onChange={e => setAiFeatures({...aiFeatures, distinctiveMarks: e.target.value})} className={`${inputStyle} h-16`} /></div>
                    </div>
