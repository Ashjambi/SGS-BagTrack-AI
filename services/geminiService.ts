...
export const analyzeFoundBaggagePhoto = async (imageUrls: string[]): Promise<{ 
    ...
    const systemInstruction = `You are a specialist at Saudi Ground Services (SGS) Baggage Analytics. 
    Analyze the provided baggage image(s). If multiple images are provided, combine insights from all views (Front, Back, Side) to build a comprehensive description.
    
    Extract the following information in JSON format:
    {
      "name": "Any visible passenger name on tags or labels (or null)",
      "description": "General visual summary",
      "features": {
        "brand": "e.g. Samsonite, American Tourister (or 'Unknown')",
        "color": "Primary color",
        "size": "Choose one: Small, Medium, Large, Extra Large",
        "type": "e.g. Suitcase, Backpack, Duffle Bag",
        "distinctiveMarks": "Specific features like stickers, ribbons, scratches, or unique patterns"
      }
    }`;
    ...
    const response = await ai.models.generateContent({
        ...
        contents: [{ 
            role: 'user',
            parts: [{ text: "Analyze this baggage from the provided view(s) and provide details in the specified JSON schema." }, ...imageParts] 
        }],
        config: { 
            systemInstruction,
